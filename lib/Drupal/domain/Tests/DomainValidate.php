...
class DomainValidate extends DomainTestBase {
  ...
  public function testDomainResponse() {
    ...
    $this->assertTrue($domain->response == 200, format_string('Server test for @url passed.', array('@url' => $domain->path->value)));
    ...
    $domain->checkResponse();
    $this->assertTrue($domain->response == 500, format_string('Server test for @url failed.', array('@url' => $domain->path->value)));
  }
}
