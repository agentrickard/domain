...
function domain_access_node_access_records(NodeInterface $node) {
  ...
  foreach ($translations as $langcode => $language) {
    ...
    if ($translation->hasField(DomainAccessManagerInterface::DOMAIN_ACCESS_ALL_FIELD) && !empty($translation->get(DomainAccessManagerInterface::DOMAIN_ACCESS_ALL_FIELD)->value) && $translation->isPublished()) {
      $grants[] = [
        'realm' => 'domain_site',
        'gid' => 0,
        'grant_view' => 1,
...
function domain_access_presave_generate(EntityInterface $entity) {
  if (!$entity->hasField(DomainAccessManagerInterface::DOMAIN_ACCESS_ALL_FIELD)) {
    return;
  }
...
function domain_access_form_node_form_alter(&$form, FormState $form_state, $form_id) {
  ...
  if (
    ...
    empty($form[DomainAccessManagerInterface::DOMAIN_ACCESS_FIELD]['#group']) &&
    empty($form[DomainAccessManagerInterface::DOMAIN_ACCESS_ALL_FIELD]['#group'])
  ) {
  ...
  switch ($context['entity_type']) {
    case 'node':
      ...
      elseif ($account->hasPermission('publish to any assigned domain')) {
        if (!empty($account->get(DomainAccessManagerInterface::DOMAIN_ACCESS_ALL_FIELD)->value)) {
          break;
        }
